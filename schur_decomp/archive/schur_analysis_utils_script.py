...
def compare_left_action_self_conditions(
    df_source_receiver: pd.DataFrame,
    normalization: str = "spectral_radius",
    target_spectral_radius: float = 0.95,
) -> dict:
    """Compare corrected Schur/eigen results with and without diagonal weights."""
    frames = {"with_self": df_source_receiver.copy()}
    no_self = df_source_receiver.copy()
    np.fill_diagonal(no_self.values, 0.0)
    frames["no_self"] = no_self
    analyses = {
        variant: prepare_left_action_schur(
            frame, normalization, target_spectral_radius
        )
        for variant, frame in frames.items()
    }
    rows, dominant_vectors = [], {}
    for variant, result in analyses.items():
        M = result["work_matrix"]
        eigvals = result["eigvals"]
        dominant_index = int(np.argmax(np.abs(eigvals)))
        dominant_vectors[variant] = result["eigvecs"][:, dominant_index]
        validation = schur_validation_summary(result, print_summary=False)
        rows.append(
            {
                "variant": variant,
                ...
                "non_normality": validation["non_normality"],
                "matrix_fro_norm": float(np.linalg.norm(M, "fro")),
                "diagonal_fro_norm": float(np.linalg.norm(np.diag(M))),
                "eigenvector_condition": validation["condition_eigenvectors"],
                "schur_reconstruction_error": validation["reconstruction_relative_error"],
            }
        )
    with_vector = dominant_vectors["with_self"]
    no_vector = dominant_vectors["no_self"]
    overlap = float(
        abs(np.vdot(with_vector, no_vector))
        / (np.linalg.norm(with_vector) * np.linalg.norm(no_vector))
    )
    summary = pd.DataFrame(rows)
    summary["dominant_vector_overlap"] = overlap
    return {
        "frames": frames,
        "analyses": analyses,
        "summary": summary,
        "dominant_vector_overlap": overlap,
        "with_self_modes": upstream_mode_summary(
            analyses["with_self"], analyses["with_self"]["labels"]
        ),
        "no_self_modes": upstream_mode_summary(
            analyses["no_self"], analyses["no_self"]["labels"]
        ),
    }
