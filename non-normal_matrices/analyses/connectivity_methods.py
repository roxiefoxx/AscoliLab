...
as the weight from sender j to receiver i, so load_connectivity_data returns J
as receiver-by-sender after transposing the CSV values.
"""
...
def load_connectivity_data(
    ...
    Returns a ``ConnectivityData`` bundle, not a bare matrix: the raw
    sender-by-receiver frame, the transposed and spectral-radius-normalized
    ``J[post, pre]``, labels, and E/I cell classes.

    Used by
    -------
    - ``run_analysis.py`` - CLI entry point for the mij paper replication
      (Shao et al. 2025 methods applied to mij_matrix.csv).
    - ``mij_paper_replication.ipynb`` - the same analysis as a notebook,
      generated by ``make_notebook.py``.

    Not to be confused with ``schur_core_script.load_mij_matrix`` in
    ``schur_decomp/``, which returns the raw untransposed ``[pre, post]``
    DataFrame and does no normalization.
